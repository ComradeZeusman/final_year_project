...
static String deviceId = WiFi.macAddress();  // Initialize with MAC address
...
if (left_sample_face == 0) {
    ...
    if(deviceId.length() > 0) {
        ...
        // Include all samples in completion
        StaticJsonDocument<32768> doc;  // Increased buffer size
        doc["deviceId"] = deviceId;
        doc["enrollmentComplete"] = true;
        doc["faceId"] = id_list.tail;
        
        JsonArray samples = doc.createNestedArray("samples");
        for(int i = 0; i < ENROLL_CONFIRM_TIMES; i++) {
            samples.add(aligned_face->item[i]);
        }

        String jsonString;
        serializeJson(doc, jsonString);

        http.begin("http://192.168.1.140:3000/enrollment/complete");
        http.addHeader("Content-Type", "application/json");
        int httpCode = http.POST(jsonString);
        
        if(httpCode > 0) {
            String response = http.getString();
            Serial.println("Enrollment Complete Response: " + response);
        }
        http.end();
    }
}
// ...existing code...
    ...
    if(deviceId.length() > 0) {
        ...
        String verify_data = "{\"deviceId\":\"" + deviceId + 
                            "\",\"faceId\":" + String(matched_id) + "}";
        http.begin("http://192.168.1.140:3000/verify");
        http.addHeader("Content-Type", "application/json");
        int httpCode = http.POST(verify_data);
...
static esp_err_t stream_handler(httpd_req_t *req) {
    ...
    while(true) {
        ...
        if(detection_enabled && fb->width <= 400) {
            ...
            // Direct JPEG streaming when detection disabled
...
static esp_err_t capture_handler(httpd_req_t *req) {
    detection_enabled = 1;  // Ensure detection is enabled
    Serial.println("Capture handler started");

    // Get camera frame
    camera_fb_t * fb = esp_camera_fb_get();
    if (!fb) {
        Serial.println("Camera capture failed");
        return httpd_resp_send(req, "{\"success\":false,\"message\":\"Camera failed\"}", -1);
    }
    ...
    if(!s) {
        ...
        dl_matrix3du_free(image_matrix);
        return httpd_resp_send(req, "{\"success\":false,\"message\":\"Format error\"}", -1);
    }
    ...
    mtmn_config.o_threshold.candidate_number = 5;
    ...
    if (!net_boxes) {
        ...
        dl_matrix3du_free(image_matrix);
        return httpd_resp_send(req, "{\"success\":false,\"message\":\"No face detected\"}", -1);
    }

    // Draw yellow box around detected face
    draw_face_boxes(image_matrix, net_boxes, 0);
    ...
    if(!fmt2jpg(image_matrix->item, fb->width*fb->height*3, fb->width, fb->height, 
        ...
        free(net_boxes->landmark);
        free(net_boxes);
        return httpd_resp_send(req, "{\"success\":false,\"message\":\"JPEG conversion failed\"}", -1);
    }

    // Send response
    httpd_resp_set_type(req, "image/jpeg");
    httpd_resp_set_hdr(req, "Content-Disposition", "inline; filename=face.jpg");
    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
...
void startCameraServer() {
    ...
    // Initialize face detection filter
...
mtmn_config.p_threshold.candidate_number = 10;  // Reduced candidates
    ...
    // Start stream server on next port
